...
tl::expected<void, std::string>
  ...
  const auto fmt_include = [](const std::filesystem::path &p, fmt::format_context &ctx) {
    return fmt::format_to(ctx.out(), "#include \"{}\"", p.string());
  };

  const auto fmt_reflected_specialization = [](const auto &refl_type, fmt::format_context &ctx) {
    // return fmt::format_to(ctx.out(), "fuck you asshole");
...
{field_names}
  ...
  const auto fmt_reflected_call = //
    [](const tool::refl::func_signature &func_sig, fmt::format_context &ctx) {
      return fmt::format_to(ctx.out(),
        R"(
template<>
...
// end)",
    fmt::arg("headers_reflected_types", util::join(data.refl_includes, "\n", fmt_include)),
    ...
    fmt::arg("reflected_calls", util::join(data.reflected_calls, "\n\n", fmt_reflected_call))
    //
  );
